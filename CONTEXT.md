...
```text
frontend/src/
├── App.jsx       # React 主入口与全部页面/面板组件；群聊成员 + 下拉面板在此实现。
└── styles.css    # 全局样式、三栏布局、聊天气泡、右侧面板、成员下拉面板和响应式规则。
```
...
- 后端扫描：37 个 `.py` 文件；PowerShell 环境中 Unix `find` 参数不可用，本次用等价 `Get-ChildItem -Recurse -Filter *.py` 扫描。
- 前端扫描：2 个 `frontend/src` 文件。
- 数据库 schema：`sqlite3 app.db ".schema"` 执行成功；本轮未新增表/列，只新增 settings seed key。
...
- 额外探针：普通 persona 权限、克隆、成员移除守卫、`/api/settings/admin.*` 拒绝、admin 登录/错误密码/admin 两个 list view 均通过。
